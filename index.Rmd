...
```{r}
income_group <- nycsat_income |>
  group_by(`Median Income`) |>
  summarise(`Median SAT Score` = median(`Average SAT Score`, na.rm = TRUE)) 


kable(income_group, digits = 0, col.names = c("Median Income", "Median SAT Score"), caption = "Median SAT Score by Median Income Group")

```
```{r}
income_group_plot <- ggplot(income_group, aes(x = `Median Income`, y = `Median SAT Score`)) +
...
```
```{r}
nycsat_race_eth_plot <- ggplot(nycsat_race_eth, aes(x = `Majority White`, y = `Median SAT Score`)) +
  geom_bar(stat = "identity") +
  labs(x = "Percentage of White Students", title = "NYC Public School SAT Scores (2014-15) by Race/Ethnicty", caption = "Sources: New York City Department of Education and College Board") +
  coord_cartesian(ylim = c(1000, 1400)) +
  theme_minimal()

nycsat_race_eth_plot
```


```{r}
race_eth_to_income <- nycsat_income |>
  group_by(`Majority White`, `Median Income`) |>
  summarise(`Median SAT Score` = median(`Average SAT Score`, na.rm = TRUE)) |>
  pivot_wider(names_from = `Median Income`,
              values_from = `Median SAT Score`)

kable(race_eth_to_income, digits = 0, caption = "Effect of Race/Ethnicity on SAT Scores Across Income groups")
  
```

```{r}
race_eth_to_income_nopiv <- nycsat_income |>
  group_by(`Majority White`, `Median Income`) |>
  summarise(`Median SAT Score` = median(`Average SAT Score`, na.rm = TRUE))
race_eth_to_income_nopiv
```

```{r}
race_eth_to_income_plot <- ggplot(race_eth_to_income_nopiv, aes(x = `Median Income`, y = `Median SAT Score`, fill = `Majority White`)) +
  geom_col(position = "dodge") +
  labs(title = "Effect of Race/Ethnicity on SAT Scores Across Income groups", caption = "Sources: New York City Department of Education, College Board, and US Census Bureau") +
  coord_cartesian(ylim = c(1000, 1400)) +
  theme_minimal()
  

race_eth_to_income_plot
```
